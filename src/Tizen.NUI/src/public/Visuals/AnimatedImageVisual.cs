    ...
    public class AnimatedImageVisual : VisualMap
    {
        ...
        protected override void ComposingPropertyMap()
        {
            if (urls != null)
            {
                ...
                _outputVisualMap.AddInt(Visual.Property.Type, (int)Visual.Type.AnimatedImage);

                if (urls.Count == 1)
                {
                    _outputVisualMap.AddString(ImageVisualProperty.URL, urls[0]);
                }
                else
                {
                    ...
                    using var temp = PropertyValue.GetReusable(urlArray);
                    _outputVisualMap.Add(ImageVisualProperty.URL, temp);
                }
                if (batchSize != null)
                {
                    _outputVisualMap.AddInt(ImageVisualProperty.BatchSize, (int)batchSize);
                }
                if (cacheSize != null)
                {
                    _outputVisualMap.AddInt(ImageVisualProperty.CacheSize, (int)cacheSize);
                }
                if (frameDelay != null)
                {
                    _outputVisualMap.AddFloat(ImageVisualProperty.FrameDelay, (float)frameDelay);
                }
                if (loopCount != null)
                {
                    _outputVisualMap.AddInt(ImageVisualProperty.LoopCount, (int)loopCount);
                }
