    ...
    public class GradientVisual : VisualMap
    {
        ...
        protected override void ComposingPropertyMap()
        {
            if (((_startPosition != null && _endPosition != null) || (_center != null && _radius != null)) && _stopColor != null)
            {
                ...
                _outputVisualMap.AddInt(Visual.Property.Type, (int)Visual.Type.Gradient);

                using var temp = PropertyValue.GetReusable(_stopColor);
                _outputVisualMap.Add(GradientVisualProperty.StopColor, temp);

                if (_startPosition != null)
                {
                    _outputVisualMap.AddVector2(GradientVisualProperty.StartPosition, _startPosition);
                }

                if (_endPosition != null)
                {
                    _outputVisualMap.AddVector2(GradientVisualProperty.EndPosition, _endPosition);
                }

                if (_center != null)
                {
                    _outputVisualMap.AddVector2(GradientVisualProperty.Center, _center);
                }

                if (_radius != null)
                {
                    _outputVisualMap.AddFloat(GradientVisualProperty.Radius, (float)_radius);
                }

                if (_stopOffset != null)
                {
                    using var stopOffsetValue = PropertyValue.GetReusable(_stopOffset);
                    _outputVisualMap.Add(GradientVisualProperty.StopOffset, stopOffsetValue);
                }

                if (_units != null)
                {
                    _outputVisualMap.AddInt(GradientVisualProperty.Units, (int)_units);
                }

                if (_spreadMethod != null)
                {
                    _outputVisualMap.AddInt(GradientVisualProperty.SpreadMethod, (int)_spreadMethod);
                }
